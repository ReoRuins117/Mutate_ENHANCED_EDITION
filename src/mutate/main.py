...
# ------------------------------------------------------------
# Helper: build a simple species template for testing
# ------------------------------------------------------------
...
def build_test_species():
    ...
    # We choose memory capacity BEFORE building BrainParams
    memory_capacity = 10

    # Compute correct neural network input size
    input_size = (
        sensory.sight_rays +
...
def main():
    ...
    print("Simulation started (backend only). Press Ctrl+C to stop.")
    ...
    try:
        while True:
            ...
            if sim.frame % 60 == 0:
                ...
                print(f"Tick {sim.frame} | Alive: {alive} | Plants: {len(world.plants)}")
        ...
        print("Logs saved to sim_log.txt")
